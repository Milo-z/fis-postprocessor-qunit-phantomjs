...
module.exports = function(content, file, settings) {
	...
	filename = "./" + (path.relative(process.cwd(), file.toString())).replace(/[\\]/g, "/");
	//filename = path.resolve(file.toString());
	run(filename, settings, function (code) {
		//process.exit(code);
	});
...
function run(filepath, options, callback) {
    var opt = options || {},
    ...
    proc = childProcess.execFile(binPath, childArgs, function (err, stdout, stderr) {
        var out,
            ...
            message,
            output;
        console.log('Testing ' + chalk.blue(filepath));
                    ...
                    try {
                        ...
                        if (out.exceptions) {
                            for (test in out.exceptions) {
								console.log(chalk.red(filepath + " Test failed"));
        ...
        if (err) {
			console.log(chalk.red(filepath + " Test error"));
            //console.log(err);
        }
    }.bind(this));
